...
const APP_BUILD_VERSION = 'revamp61-supabase-pdf-preview-zoom';
...
function persistManualRequestsFromRemote(items) {
    ...
    const approvedStores = normalized
        ...
        address: item.address,
        city: '',
        source: supabaseEnabled() ? 'supabase-approved' : 'convex-approved',
...
function SecretMonitorPanel({ open, onClose, history, welcomeConfig, onWelcomeConfigChange }) {
    ...
    async function saveWelcomeSettings() {
        ...
        alert(synced ? 'Text welcome berhasil disimpan dan disinkronkan ke Supabase.' : 'Text welcome tersimpan lokal, tapi Supabase belum aktif atau gagal sync.');
    }
    ...
    async function saveNoticeSettings() {
        ...
        alert(synced ? 'Informasi update HOME berhasil disimpan dan disinkronkan ke Supabase.' : 'Informasi update tersimpan lokal, tapi Supabase belum aktif atau gagal sync.');
    }
    ...
    async function refresh(options = {}) {
        ...
        try {
            ...
            if (remoteRows !== null) {
                applyRows(remoteRows, supabaseEnabled() ? 'supabase' : (source === 'convex realtime' ? 'convex realtime' : 'convex'));
            }
            else {
    ...
    const isLive = source === 'supabase' || source === 'convex realtime';
    const sourceBadgeLabel = source === 'supabase' ? 'Supabase Sync' : source === 'convex realtime' ? 'Live Convex' : 'Manual refresh';
