(function () {
    ...
    return new Promise(function (resolve) {
      ...
      img.onload = function () {
        try {
          ...
          resolve({ dataUrl: canvas.toDataURL('image/jpeg', 0.88), width: width, height: height });
  ...
  function buildPhotoGridItems(doc, photos, cardWidth, cardHeight) {
    ...
    const imageHeight = Math.min(42, cardHeight * 0.66);
    ...
    photos.forEach(function (photo, index) {
      ...
      while (rest.length) {
        ...
        items.push({ image: '', lines: chunk, lineHeight: lineHeight, imageHeight: 0, title: 'Lanjutan deskripsi Foto ' + String(index + 1) + '.' + String(continuationIndex), continuation: true });
  ...
  async function drawPhotoGridCard(doc, item, x, y, width, height, palette) {
    ...
    if (item.image) {
      ...
      const imgY = y + 4;
      const imgW = width - 8;
      const imgH = item.imageHeight;
      const added = await addImageInBox(doc, item.image, imgX, imgY, imgW, imgH, 'contain');
      if (!added) {
        doc.setFillColor(248, 250, 252);
