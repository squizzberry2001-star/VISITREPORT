...
function RichTextInput({ value, onChange, placeholder = 'Tulis catatan...', className = '', minHeight = 112 }) {
  ...
  useEffect(() => {
    ...
    if (editor.innerHTML !== richValue(value)) editor.innerHTML = richValue(value);
  }, [value]);
  ...
  function command(name, argument = null) {
    ...
    window.requestAnimationFrame(() => {
      ...
      ['bold', 'italic', 'underline', 'insertUnorderedList', 'insertOrderedList'].forEach((tool) => {
        try { next[tool] = document.queryCommandState(tool); } catch (error) { next[tool] = false; }
      });
      setActiveTools(next);
    });
  }
      ...
      <div className="rich-toolbar flex flex-wrap gap-1 border-t border-slate-200 p-2" aria-label="Rich text toolbar">
        {tools.map(([name, label, title]) => (
          <button
            key={name}
            type="button"
            className={cx('rich-tool-button', activeTools[name] && 'active')}
            onMouseDown={(event) => { event.preventDefault(); command(name); }}
            onTouchStart={(event) => { event.preventDefault(); command(name); }}
            aria-label={title}
            title={title}
          >
            {label}
  ...
  return (
    <div className="photo-editor-overlay fixed inset-0 z-[95] grid place-items-center bg-slate-950/60 p-3 backdrop-blur-[2px]" role="dialog" aria-modal="true">
      <div className="photo-editor-panel w-full max-w-2xl rounded-[28px] bg-white p-4 shadow-2xl md:p-5">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-950 p-2"><canvas ref={canvasRef} width="1200" height="820" className="mx-auto block w-full touch-none rounded-xl bg-white" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onWheel={handleWheel} /></div>
        ...
        <article key={index} className="surface-card rounded-[28px] p-4 md:p-5">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="grid gap-4 sm:grid-cols-[170px_1fr]">
              <Field label="Deadline"><TextInput type="date" value={row.deadline || ''} onChange={(e) => updateRow(index, { deadline: e.target.value })} /></Field>
