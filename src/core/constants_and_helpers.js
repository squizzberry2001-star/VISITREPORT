...
const SESSION_SCREEN_KEY = 'rbv_session_screen_v109';
...
const EMAIL_TEMPLATE_ADMIN_KEY = 'rbv_email_template_admin_v109';
...
const APP_BUILD_VERSION = 'revamp327-modular-v109';
...
function getNativeCameraCaptureAttr() {
    // Kembalikan nilai 'environment' hanya untuk iOS/Desktop.
    // Untuk Android: banyak brand (Samsung, Xiaomi, Huawei, Oppo, dll.) crash
    // atau freeze jika atribut capture="camera" dipaksakan melalui PWA/WebView.
    // Solusi: hapus atribut capture (return undefined) agar OS yang memilih
    // apakah membuka kamera atau galeri — ini lebih kompatibel.
    try {
        const ua = navigator?.userAgent || '';
        if (/Android/i.test(ua)) return undefined; // biarkan OS yang handle
        return 'environment'; // iOS & desktop: prefer rear camera
    } catch (error) {
        return undefined;
    }
}
function rbvPrepareCameraCapture() {
    ...
    try {
        ...
        // JANGAN panggil rbvReleaseCameraResources() di sini!
        // Memanggil track.stop() sebelum picker terbuka akan membunuh akses
        // kamera di banyak Android (Samsung, Xiaomi, Oppo, Vivo, Realme).
        // Resource akan dibebaskan SETELAH picker selesai di rbvFinishCameraCapture().
    } catch (error) {}
}
